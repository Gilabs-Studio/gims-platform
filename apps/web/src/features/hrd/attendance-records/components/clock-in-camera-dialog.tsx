...
export function ClockInCameraDialog({
  ...
  const { isDenied: isLocationDenied, requestPermissionOrFallback } =
  ...
  const handleConfirm = useCallback(async () => {
    ...
    try {
      // Convert data URL to Blob directly (no fetch needed)
      const blob = dataURLtoBlob(capturedImage);
      const file = new File(
        [blob],
        `attendance-${checkInType.toLowerCase()}-${Date.now()}.jpg`,
        {
          type: "image/jpeg",
        },
      );
  ...
  }, [capturedImage, checkInType, onConfirm, t, dataURLtoBlob]);
