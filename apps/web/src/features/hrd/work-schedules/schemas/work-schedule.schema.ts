...
export const workScheduleSchema = z.object({
  ...
  flexible_start_time: z.string()
    .regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, "Invalid time format (HH:MM)")
    .optional(),
  flexible_end_time: z.string()
    .regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, "Invalid time format (HH:MM)")
    .optional(),
  ...
  message: "Flexible start and end times are required when flexible schedule is enabled",
  path: ["flexible_start_time"],
}).refine((data) => {
  // If GPS required, coordinates should be set
  if (data.require_gps) {
    return data.office_latitude !== undefined && data.office_longitude !== undefined;
  }
  return true;
}, {
...
export type UpdateWorkScheduleFormData = z.infer<typeof updateWorkScheduleSchema>;
