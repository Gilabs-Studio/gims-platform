...
import { usePurchasePayment, usePurchasePaymentAuditTrail } from "../hooks/use-purchase-payments";
...
export function PurchasePaymentDetail({ open, onClose, paymentId }: PurchasePaymentDetailProps) {
  ...
  const { data: auditData, isLoading: auditLoading, isError: auditError } = usePurchasePaymentAuditTrail(
    paymentId ?? "",
    { page: auditPage, per_page: auditPageSize },
    { enabled: open && !!paymentId && canAuditTrail && activeTab === "audit-trail" },
  );
        ...
        {isFetching ? (
          ...
          >
            <TabsList>
              ...
              {canAuditTrail && <TabsTrigger value="audit-trail">{t("tabs.auditTrail")}</TabsTrigger>}
            </TabsList>
