...
import { useCreateProductBrand, useUpdateProductBrand } from "./use-product-brands";
...
export type ProductBrandFormData = z.infer<typeof productBrandFormSchema>;

export interface UseProductBrandFormProps {
  open: boolean;
...
export function useProductBrandForm({ open, onOpenChange, editingItem }: UseProductBrandFormProps) {
  ...
  const form = useForm<ProductBrandFormData>({
  ...
  const onSubmit: SubmitHandler<ProductBrandFormData> = async (data) => {
    try {
      ...
      toast.error(isEditing ? tCommon("error") : "Failed to create product brand");
    }
  };
