...
function findActionByCode(
  ...
  for (const menu of menus) {
    // Check if this menu or its children are related to company
    const isCompanyMenu =
      menu.url?.includes("/company") ||
      menu.name?.toLowerCase().includes("company") ||
      (menu.children?.some((child) =>
        child.url?.includes("/company") || child.name?.toLowerCase().includes("company")
      ) ?? false);
...
/**
 * Hook to check if the current user has a specific permission for COMPANY menu
...
export function useHasPermission(permissionCode: string): boolean {
  ...
  const hasPermission = useMemo(() => {
    ...
    // Search for the action in COMPANY-related menus
