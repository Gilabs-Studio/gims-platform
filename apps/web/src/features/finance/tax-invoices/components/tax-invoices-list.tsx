...
export function TaxInvoicesList() {
  ...
  const canCreate = useUserPermission("tax_invoice.create");
  const canUpdate = useUserPermission("tax_invoice.update");
  const canDelete = useUserPermission("tax_invoice.delete");
  ...
  const [deleting, setDeleting] = useState<TaxInvoice | null>(null);

  const { data, isLoading, isError } = useFinanceTaxInvoices({
    page,
    per_page: pageSize,
    search: debouncedSearch || undefined,
    sort_by: "tax_invoice_date",
    sort_dir: "desc",
  });

  const rows = useMemo(() => data?.data ?? [], [data?.data]);
  const pagination = data?.meta?.pagination;
  const deleteMutation = useDeleteFinanceTaxInvoice();
  ...
  return (
    <div className="space-y-6">
      ...
      <TaxInvoiceForm open={formOpen} onOpenChange={setFormOpen} mode={formMode} initialData={editing} />
