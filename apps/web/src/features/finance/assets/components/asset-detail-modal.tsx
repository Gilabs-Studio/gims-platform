...
export function AssetDetailModal({
  ...
  // Phase 2 hooks
  const { data: attachmentsData } = useAssetAttachments(assetId ?? "", {
    enabled: !!assetId && open,
  });
  const { data: auditLogsData } = useAssetAuditLogs(assetId ?? "", {
    enabled: !!assetId && open,
  });
  const { data: assignmentHistoryData } = useAssetAssignmentHistory(
    assetId ?? "",
    { enabled: !!assetId && open },
  );
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // toast is not imported in this component - the mutation onSuccess handles invalidation
    } catch {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent size="xl" className="max-h-[90vh] flex flex-col p-0">
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-6">
          {isLoading ? (
            ...
            <Tabs defaultValue="overview" className="w-full">
              <TabsList className="flex w-full h-auto overflow-x-auto overflow-y-hidden flex-row items-center justify-start gap-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] pb-1 sticky top-0 bg-background z-10 border-b mb-4">
                          ...
                          <TableBody>
                            ...
                            {asset.is_under_warranty &&
                              asset.warranty_days_remaining > 0 && (
                                <InfoRow
                                  label={t(
                                    "detail.fields.warrantyDaysRemaining",
                                  )}
                  ...
                  <div className="border rounded-lg overflow-x-auto">
                    <Table>
                      <TableHeader>
                        <TableRow>
                          <TableHead>
                            {t("detail.depreciation.period")}
                          </TableHead>
                          <TableHead>{t("detail.depreciation.date")}</TableHead>
                          <TableHead>
                            {t("detail.depreciation.method")}
                          </TableHead>
                  ...
                  <div className="border rounded-lg overflow-x-auto">
                    <Table>
                      <TableHeader>
                        <TableRow>
                          <TableHead>
                            {t("detail.attachments.fileName")}
                          </TableHead>
                          <TableHead>
                            {t("detail.attachments.fileType")}
                          </TableHead>
                          <TableHead>
                            {t("detail.attachments.fileSize")}
                          </TableHead>
                          <TableHead>
                            {t("detail.attachments.uploadedAt")}
                          </TableHead>
                          <TableHead>
                  ...
                  <div className="border rounded-lg overflow-x-auto">
                    <Table>
                      <TableHeader>
                        <TableRow>
                          <TableHead>
                            {t("detail.assignmentHistory.employee")}
                          </TableHead>
                          <TableHead>
                            {t("detail.assignmentHistory.assignedAt")}
                          </TableHead>
                          <TableHead>
                            {t("detail.assignmentHistory.returnedAt")}
                          </TableHead>
                          <TableHead>
                            {t("detail.assignmentHistory.status")}
                          </TableHead>
                          <TableHead>
                            {t("detail.assignmentHistory.notes")}
                          </TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
              ...
              <TabsContent value="depreciation-config" className="py-4">
                <AssetDepreciationConfigTab
                  ...
                  onUpdateConfig={async (config) => {
                    // TODO: Implement API call to update depreciation config
              ...
              <TabsContent value="components" className="py-4">
                <AssetComponentsTab
                  asset={asset}
                  isLoading={isLoading}
                  onAddComponent={async (componentId) => {
                    // TODO: Implement API call to add component
                    toast.success(t("toast.componentAdded"));
                  }}
                  onRemoveComponent={async (componentId) => {
                    // TODO: Implement API call to remove component
