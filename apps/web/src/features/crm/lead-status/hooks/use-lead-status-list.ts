...
import { useLeadStatuses, useDeleteLeadStatus, useUpdateLeadStatus } from "./use-lead-status";
import { useUserPermission } from "@/hooks/use-user-permission";
import type { LeadStatus } from "../types";

export function useLeadStatusList() {
  const t = useTranslations("leadStatus");
  const tCommon = useTranslations("common");

  const canCreate = useUserPermission("crm_lead_status.create");
  const canUpdate = useUserPermission("crm_lead_status.update");
  const canDelete = useUserPermission("crm_lead_status.delete");
  ...
  const handleEdit = (item: LeadStatus) => { setEditingItem(item); setDialogOpen(true); };
