export const crmDealEn = {
  crmDeal: {
    title: "Pipeline & Deals",
    createTitle: "Add Deal",
    editTitle: "Edit Deal",
    detailTitle: "Deal Detail",
    created: "Deal created successfully",
    updated: "Deal updated successfully",
    deleted: "Deal deleted successfully",
    stageMoved: "Deal stage moved successfully",
    addDeal: "Add Deal",
    searchPlaceholder: "Search deals...",
    emptyState: "No deals found",
    noItems: "No product items",
    noHistory: "No stage transition history",
    activities: "Activities",
    activityTimeline: "Activity Timeline",
    noActivities: "No activities yet",
    addActivity: "Log Activity",
    logVisit: "Log Visit",
    information: "Information",
    tasks: "Tasks",
    noTasks: "No tasks yet",
    createTask: "Create Task",
    addTask: "Add Task",
    visitReport: "Visit Report",
    visitReports: "Visit Reports",
    noVisitReports: "No visit reports",
    noLocation: "No location set",
    locationUpdated: "Location updated successfully",
    setLocation: "Set Location",
    updateLocation: "Update Location",

    // Actions
    edit: "Edit",
    delete: "Delete",
    cancel: "Cancel",
    save: "Save",
    saving: "Saving...",
    deleting: "Deleting...",
    moving: "Moving...",
    moveStage: "Move Stage",
    viewDetail: "View Detail",

    // Delete dialog
    deleteDealTitle: "Delete Deal",
    deleteDealDescription:
      "Are you sure you want to delete this deal? This action cannot be undone.",

    // Move stage dialog
    moveStageTitle: "Move Deal Stage",
    moveStageDescription: 'Move "{title}" to a new pipeline stage',
    currentStage: "Current Stage",
    targetStage: "Target Stage",
    selectStage: "Select target stage",
    moveStageConvertLabel: "Convert to Sales Quotation",
    moveStageConvertDescription: "Automatically create a sales quotation from this deal upon moving to Won stage.",
    moveStageConvertNoItems: "Cannot convert: deal has no product items.",
    moveStageConvertNoCustomer: "Cannot convert: deal has no customer or linked lead.",
    moveStageConvertAutoCreateCustomer: "A new customer will be auto-created from the linked lead data.",
    convertedToSQBadge: "Converted to SQ",

    // Kanban
    kanbanTitle: "Pipeline Board",
    summaryTitle: "Pipeline Summary",
    createDeal: "Create Deal",
    updateDeal: "Update Deal",
    noDeals: "No deals in this stage",
    noPipelineStages: "No pipeline stages configured",

    // Form fields
    dealTitle: "Deal Title",
    dealTitlePlaceholder: "Enter deal title",
    description: "Description",
    descriptionPlaceholder: "Describe the deal",
    value: "Value",
    valuePlaceholder: "0",
    valueAutoCalculated: "Value is auto-calculated from product items",
    probability: "Probability (%)",
    probabilityPlaceholder: "0",
    pipelineStage: "Pipeline Stage",
    pipelineStagePlaceholder: "Select pipeline stage",
    customer: "Customer",
    potential: "Prospect",
    prospectCustomerPlaceholder: "Prospect customer from lead",
    prospectCustomerHint: "This deal comes from lead conversion, customer will be created during downstream conversion.",
    customerPlaceholder: "Select customer",
    selectCustomer: "Select customer",
    contact: "Contact",
    prospectContactPlaceholder: "Contact from lead",
    prospectContactHint: "Contact data follows lead basic and contact information.",
    contactPlaceholder: "Select contact",
    selectContact: "Select contact",
    fromLead: "From lead",
    location: "Location",
    openInMaps: "Open in Maps",
    assignedTo: "Assigned To",
    prospectAssignmentPlaceholder: "Assigned employee from lead",
    prospectAssignmentHint: "Assignment follows lead conversion data.",
    assignedToPlaceholder: "Select employee",
    selectEmployee: "Select employee",
    lead: "Lead",
    prospectLeadPlaceholder: "Linked lead",
    prospectLeadHint: "Lead is locked because this deal was converted from that lead.",
    leadPlaceholder: "Select lead (optional)",
    selectLead: "Select lead (optional)",
    bankAccount: "Linked Bank Account",
    bankAccountPlaceholder: "Select linked bank account",
    bankAccountOwnerType: {
      company: "Company",
      customer: "Customer",
      supplier: "Supplier",
    },
    bankAccountReference: "Bank Account Reference",
    bankAccountReferencePlaceholder: "e.g. BCA 1234567890 a.n PT Example",
    expectedCloseDate: "Expected Close Date",
    expectedClose: "Expected Close",
    actualClose: "Actual Close",
    createdAt: "Created",
    notes: "Notes",
    notesPlaceholder: "Additional notes about this deal",
    reason: "Reason",
    reasonPlaceholder: "Reason for stage change",
    closeReason: "Close Reason",
    closeReasonPlaceholder: "Reason for closing this deal",
    dates: "Dates",

    // Product items
    productItems: "Product Items",
    addItem: "Add Product Item",
    product: "Product",
    productPlaceholder: "Select product",
    selectProduct: "Select product",
    unitPrice: "Unit Price",
    sku: "SKU",
    qty: "Quantity",
    discountPct: "Discount %",
    itemNotes: "Notes",
    subtotal: "Subtotal",
    total: "Total",
    productsTotal: "Products Total",
    bantBudget: "BANT Budget",
    interest: "Interest",
    removeItem: "Remove item",
    restoreItem: "Restore item",

    // BANT
    bantTitle: "BANT Qualification",
    budget: "Budget",
    authority: "Authority",
    need: "Need",
    timeline: "Timeline",
    budgetConfirmed: "Budget Confirmed",
    authConfirmed: "Authority Confirmed",
    needConfirmed: "Need Confirmed",
    timeConfirmed: "Timeline Confirmed",
    budgetAmount: "Budget Amount",
    budgetAmountPlaceholder: "0",
    authPerson: "Authority Person",
    authPersonPlaceholder: "Enter authority person name",
    needDescription: "Need Description",
    needDescriptionPlaceholder: "Describe the need",

    // History
    history: "History",
    probabilityChange: "Probability",
    daysInPreviousStage: "{days} days in previous stage",
    by: "by",

    // Summary
    totalDeals: "Total Deals",
    openDeals: "Open Deals",
    wonDeals: "Won Deals",
    lostDeals: "Lost Deals",
    winRate: "Win Rate",
    winRateDescription: "{won} won out of {total} closed deals",
    stageBreakdown: "Stage Breakdown",

    // Form tabs & section headers
    tabBasic: "Basic Information",
    tabItems: "Products & BANT",
    sectionDealInfo: "Deal Information",
    sectionCustomerInfo: "Customer & Assignment",
    summary: "Summary",
    btnNext: "Next",
    btnBack: "Back",
    selectDate: "Select date",
    dealAlreadyClosed: "This deal is already closed (won/lost) and cannot be edited.",
    errorAlreadyClosed: "Deal is already closed and cannot be modified.",

    // Sections
    sections: {
      basicInfo: "Basic Information",
      productItems: "Product Items",
      bant: "BANT Qualification",
      assignment: "Assignment",
      notes: "Notes",
    },

    // Validation
    validation: {
      titleRequired: "Deal title is required",
      titleMin: "Title must be at least 2 characters",
      titleMax: "Title must be at most 200 characters",
      stageRequired: "Pipeline stage is required",
      customerRequired: "Customer is required",
      targetStageRequired: "Target stage is required",
    },

    // Conversion (Sprint 21)
    conversion: {
      convertToQuotation: "Convert to Quotation",
      title: "Convert Deal to Sales Quotation",
      description: 'Convert "{title}" into a sales quotation with all product items and pricing.',
      confirm: "Convert to Quotation",
      converting: "Converting...",
      convertSuccess: "Deal converted to quotation successfully",
      convertError: "Failed to convert deal to quotation",
      quotationCreated: "A sales quotation has been created from this deal.",
      quotationCode: "Quotation Code",
      viewQuotation: "View Quotation",
      alreadyConverted: "This deal has been converted to a sales quotation.",
      permissionRequired: "You don't have permission to convert deals",
      notesLabel: "Additional Notes (Optional)",
      notesPlaceholder: "Enter any additional notes for the quotation...",
    },

    // Stock Check (Sprint 21)
    stock: {
      checkStock: "Check Stock",
      checking: "Checking stock availability...",
      checkError: "Failed to check stock",
      retry: "Retry",
      refresh: "Refresh",
      allSufficient: "All items in stock",
      someInsufficient: "Some items have insufficient stock",
      requested: "Requested",
      available: "Available",
      status: "Status",
    },
  },
};
