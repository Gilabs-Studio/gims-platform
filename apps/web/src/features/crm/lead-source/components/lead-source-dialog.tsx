      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field><FieldLabel>{t("form.code")}</FieldLabel><Input placeholder={t("form.codePlaceholder")} {...register("code")} />{errors.code && <FieldError>{errors.code.message}</FieldError>}</Field>
          </div>
          <Field><FieldLabel>{t("form.order")}</FieldLabel><Input type="number" min={0} {...register("order", { valueAsNumber: true })} />{errors.order && <FieldError>{errors.order.message}</FieldError>}</Field>
